...
ekf_theta.x [2] += random.random() * 0.05 """
    ...
    while viewer.is_running() and (not exit) :
        ...
        if not pause or step:
            ...
            with viewer.lock():
                ...
                #c_t_est = - l_1 / 2 * np.array([math.sin(q_est), 0, math.cos(q_est)]) 
                c_t_est = l_est * np.array([math.sin(q_est), 0, math.cos(q_est)])    
...
ax[2].plot(sim_time, l_1 * np.ones(len(sim_time)))
ax[2].plot(sim_time, est_len)
ax[2].legend(['sim', 'est'])
