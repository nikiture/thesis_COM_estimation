...
model = mujoco.MjModel.from_xml_path ("models/case_2_encoder.xml")
...
ekf_theta = Modif_EKF(4, 4, 2)

ekf_theta.x [0] = data.qpos[0].copy()
ekf_theta.x [1] = data.qpos[1].copy()

ekf_theta.x [2] = data.qvel[0].copy()
ekf_theta.x [3] = data.qvel[1].copy()
...
def compute_z (data, x):
    ang_data_1 = data.sensor ("IMU_gyro").data[1].copy() 
                
    acc_data_1 = data.sensor ("IMU_acc").data.copy()
    
    enc_data = data.sensor("angle_2_sens").data[0].copy()
    ...
    mujoco.mju_mulMatVec (acc_data_1, R, acc_data_1.copy())
    ...
    z = np.array([ang_data_1, acc_data_1[0], acc_data_1[2], enc_data])
