...
model = mujoco.MjModel.from_xml_path("models/3D_prop_base.xml")
...
start_angle_3 = - 0
start_axis_3 = np.zeros(3)
start_axis_3[1] = 1
mujoco.mju_normalize(start_axis_3) 

mujoco.mju_axisAngle2Quat(data.qpos[:4], start_axis_1, start_angle_1)
data.qpos[4] = start_angle_2
mujoco.mju_axisAngle2Quat(data.qpos[5:9], start_axis_3, start_angle_3)
...
l = np.zeros(0)
#l_1 = 1
l = np.append(l, mujoco.mju_dist3(data.joint("ground_joint").xanchor, data.joint("middle_joint").xanchor))
l = np.append(l, mujoco.mju_dist3(data.joint("middle_joint").xanchor, data.site("stick_end").xpos))
    ...
    if not pause:
        ...
        for i, jname in enumerate(joint_names):
            ...
            if jmod.type == mujoco.mjtJoint.mjJNT_BALL:
                ...
                mujoco.mju_mulMatTVec(q_curr[1:], R_0_1, q_curr[1:].copy()) 
