...
model = mujoco.MjModel.from_xml_path("models/suspended_double_prop.xml")
...
start_angle_2 = 0
start_axis_2 = np.zeros(3)
start_axis_2[0] = 1
mujoco.mju_normalize(start_axis_2) 
...
R_0_2 = np.eye(3)
mujoco.mju_mulMatMat(R_0_2, R_0_1, R_1_2)
#R_0_2 = R_0_1 @ R_1_2
...
R_2_3 = np.eye(3)
R_0_3 = np.eye(3)
mujoco.mju_mulMatMat(R_0_3, R_0_2, R_2_3)
T_2_3 = np.eye(4)
T_2_3 [0, 3] = l [1]
...
def_R_0_i = np.array([R_0_1, R_0_2, R_0_3])
    ...
    while viewer.is_running() and not exit:
        ...
        if not pause:
            ...
            for i, jname in enumerate(joint_names):
                ...
                mujoco.mju_mulMatMat(R_0_i, R_0_i.copy(), def_R_0_i[i])
                #print (jmod.type)
                ...
                if jmod.type == mujoco.mjtJoint.mjJNT_BALL:
                    ...
                    #mujoco.mju_mulMatTVec(q_curr[1:], R_0_1.copy(), q_curr[1:].copy()) 
                ...
                if jmod.type == mujoco.mjtJoint.mjJNT_FREE:
                    ...
                    mujoco.mju_mulMatVec(q_curr[1:], R_0_i.copy(), q_curr[1:].copy()) 
                    csi_cross = skew_mat(q_curr[1:])
                    csi_cross_sq = np.eye(3)
                    mujoco.mju_mulMatMat(csi_cross_sq, csi_cross, csi_cross)
                    R_i_i [i] = np.eye(3) + 2 * q_curr[0] * csi_cross + 2 * csi_cross_sq
                ...
                mujoco.mju_mulMatMat(R_0_i, R_0_i.copy(), R_i_i [i])
                #R_0_i = R_0_i @ R_i_i [i]
