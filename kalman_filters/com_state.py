...
def h_x (x):
    l_imu = (l_2 / 2)
    l_c = math.sqrt (x[0]**2 + x[1]**2)
    H = np.zeros (3)
    
    """ H [1] = - l_imu * (Kr * g * x[0] * x[1] / l_c**2)
    H [2] = l_imu * (Kr * g * (x[0] / l_c)**2)
    
    if abs(ekf_COM.x[1]) >= threshold:
        H [0] = x[2] / x [1]
        H [2] -= l_imu / l_c * (x[2] * x[3] / x[1])
    
    if abs (ekf_COM.x[0]) >= threshold:
        H [0] = x[3] / x[0]
        H [1] += l_imu / l_c * (x[3] * x[2] / x[0]) """
    """ H [1] = - l_imu * (Kr * g * x[0] * x[1] / l_c**2)
    H [2] = l_imu * (Kr * g * (x[0] / l_c)**2)
    
    if abs(ekf_COM.x[1]) >= threshold:
        H [0] = - x[2] / x [1]
        H [1] += l_imu / l_c * (x[2] * x[3] / x[1])
    
    if abs (ekf_COM.x[0]) >= threshold:
        H [0] = x[3] / x[0]
        H [2] -= l_imu / l_c * (x[3] * x[2] / x[0]) """
    
    #print((x[0] * x[3] - x[1] * x[2]) / l_c**2 - math.sqrt(x[2]**2 + x[3]**2) / l_c)
    
    H [0] = (x[0] * x[3] - x[1] * x[2]) / l_c**2
    #H [0] = math.sqrt(x[2]**2 + x[3]**2) / l_c
    
    H [1] = l_imu / l_c**3 * (x[0] * x[1] * g - x[0] * x[2]**2 - x[0] * x[3]**2)
    
    H [2] = l_imu / l_c**3 * (-x[0]**2 * g - x[1] * x[2]**2 - x[1] * x[3]**2)
...
def H_jac (x):
    ...
    l_c = math.sqrt (x[0]**2 + x[1]**2)
    l_c_2 = x[0]**2 + x[1]**2
    v = math.sqrt(x[2]**2 + x[3]**2)
    H = np.zeros((3, 4))
    
    """ H [1, 0] = - l_imu / l_c**3 * Kr * g * x[1]
    H [1, 1] = - l_imu / l_c**3 * Kr * g * x[0]
    
    H [2, 0] = 2 * l_imu / l_c**3 * Kr * g * x[0]
    
    if abs(x[1]) >= threshold:
        
        #H [0, :] = np.array ([0, x[2] / x[1]**2, - 1 / x[1], 0]).reshape((1, 4))
        H [0, 0] = 0
        H [0, 1] = x[2] / x[1]**2
        H [0, 2] = - 1 / x[1]
        H [0, 3] = 0
    ...
    if abs (x[0]) >= threshold:
        #H [0, :] = np.array ([- x[3] / x[0]**2, 0, 0, 1 / x[0]]).reshape ((1, 4))
        H [0, 0] = - x[3] / x[0]**2
        H [0, 1] = 0
        H [0, 2] = 0
        H [0, 3] = 1 / x[0]
    ...
    """ H [1, 0] = - 2 * l_imu * (Kr * g * x[1] + x[3]**2) / l_c**3 + 4 * (x[0]**2 * (Kr * g * x[1] + x[3]**2) + x[1]**2 * x[3]**2) / l_c**5
    
    H [1, 1] = - l_imu / l_c**3 * Kr * g * x[0] + l_imu / l_c**5 * 3 * x[0] * (Kr * g * x[1]**2 + x[1] * x[3]**2)
    
    H [2, 0] = 2 * l_imu * Kr * g * x[0] / l_c**3
    H [2, 0] -= 3 * l_imu * x[0] * (x[0]**2 * Kr * g - x[1] * x[2]**2) / l_c**5
    
    H [2, 1] = l_imu / l_c**5 * (4 * (x[1]**2 * x[2]**2 - Kr * g * x[1] * x[0]**2) + 5 * x[0]**2 * x[2]**2)
    H [2, 1] -= l_imu / l_c**3 * 2 * x[2]**2
    ...
    if abs(x[1]) >= threshold:
        ...
        H [2, 0] += 3 * l_imu * x[0]**3 * x[2]**2 / (l_c**5 * x[1])
        H [2, 0] -= 2 * l_imu * x[0] * x[2]**2 / (l_c**3 * x[1])
        
        H [2, 1] += l_imu / l_c**5 * (x[0]**4 * (x[2]**2 - Kr * g * x [1])) / x[1]**2
        H [2, 1] += l_imu / l_c**3 * Kr * g * x[0]**2 / x[1]
        
        H [2, 2] = - l_imu / l_c * 2 * x[2] / x[1]
    ...
    if abs (x[0]) >= threshold:
        ...
        H [1, 0] += l_imu * (x[1]**2) * (x[0]**2 * (Kr * g * x[1] + x[3]**2) + x[1]**2 * x[3]**2) / (x[0]**2 * l_c**5)
        H [1, 3] = - l_imu / l_c * 2 * x[3] / x[0]
        
        H[1, 1] += 3 * l_imu * x[1]**3 * x[3]**2 / (x[0] * l_c**5) - 2 * l_imu * x[1] * x[3]**2 / (l_c**3 * x[0]) """
        
    """ H [1, 0] = l_imu * Kr * g * (x[0]**2 - x[1]**2) * x[1] / l_c_2**2
    
    H [1, 1] = l_imu * (Kr * g * (x[0] * x[1]**2 - x[0]**3) / l_c_2**2 + 2 * x[2] * x[3] / l_c) 
    
    H [2, 0] = l_imu * Kr * g * 2 * x[0] * x[1]**2 / l_c_2**2
    H [2, 0] -= 2 * l_imu * x[2] * x[3] / l_c**3
    
    H [2, 1] = - l_imu * (2 * Kr * g * x[1] * x[0]**2) / l_c_2**2
    ...
    if abs(x[1]) >= threshold:
        ...
        H [1, 0] += l_imu / l_c**3 * x[0] * x[2] * x[3] / x[1]
        
        H [1, 1] += l_imu / l_c * x[0]**2 * x[2] * x[3] / x[1]**2 
        
        H [1, 2] = - l_imu / l_c * x[3] / x[1]
        
        H [1, 3] = - l_imu / l_c * x[2] / x[1]
    ...
    if abs (x[0]) >= threshold:
        ...
        H [2, 0] -= l_imu * x[1]**2 * x[3]  * x[2] / (x[0]**2 * l_c**3)
        
        H [2, 1] -= l_imu / l_c**3 * x[2] * x[3] * x[1]**2 / x[0]**2
        
        H [2, 2] += l_imu / l_c * x[3] / x[0]
        
        H [2, 3] += l_imu / l_c * x[2] / x[0] """
    ...
    if abs (x[0]) >= threshold:
        ...
        H [2, 1] += l_imu / l_c**3 * x[2] * x[3] * x[1] / x[0]
        
        H [2, 2] -= l_imu / l_c * x[3] / x[0]
        
        H [2, 3] -= l_imu / l_c * x[2] / x[0] """
    
    H [0, 0] = (2 * x[0] * x[1] * x[2] - (x[0]**2 - x[1]**2) * x[3]) / l_c_2
    
    H [0, 1] = (2 * x[0] * x[1] * x[3] - (x[0]**2 - x[1]**2) * x[2]) / l_c_2
    ...
    H [1, 0] = - l_imu * (2 * x[0]**2 - x[1]**2) * (x[1] * g - x[2]**2 - x[3]**2) / l_c**5
    
    H [1, 1] = l_imu * x[0] * ((x[0]**2 - 2 * x[1]**2) * g + 3 * x[1] * (x[2]**2 + x[3]**2)) / l_c**5
    
    H [1, 2] = - 2 * l_imu * x[0] * x[2] / l_c**3
    H [1, 3] = - 2 * l_imu * x[0] * x[3] / l_c**3
    
    
    H [2, 0] = l_imu * x[0] * (3 * (x[0]**3 * g + x[1] * (x[2]**2 + x[3]**2)) / l_c**5 - 2 * x[0] * g / l_c**3)
    
    H [2, 1] = - l_imu * ((x[0]**2 - 2 * x[1]**2) * (x[2]**2 + x[3]**2) + 3 * x[0]**2 * x[1] * g) / l_c**5
    
    H [2, 2] = - 2 * l_imu * x[1] * x[2] / l_c**3
    H [2, 3] = - 2 * l_imu * x[1] * x[3] / l_c**3
...
dt = model.opt.timestep * count_max
 ...
 #term collected during computations, computed from Lc^2 * m / I

ekf_COM = ExtendedKalmanFilter (4, 3) #1 dim for angular velocity, 2 for acceleration

start_angle = data.qpos[0] + 0.05 * random.random()
ekf_COM.x[0] = l_2 / 2 * math.sin(start_angle)
ekf_COM.x[1] = -l_2 / 2 * math.cos(start_angle)


#print (ekf_COM.x[0], data.qpos[0])

""" ekf_COM.R [0, 0] = model.sensor("IMU_gyro").noise
ekf_COM.R [1, 1] = model.sensor("IMU_acc").noise
ekf_COM.R [2, 2] = model.sensor("IMU_acc").noise """
...
#print (ekf_COM.R)
#ekf_COM.Q = Q_discrete_white_noise (3, dt = dt, var = 0.0001)
#ekf_COM.Q *= 0.0002
#ekf_COM.Q *= 0
ekf_COM.R *= 0.0001
ekf_COM.Q *= 0.0001
    ...
    while viewer.is_running() and not exit:
        ...
        if (not pause) or step:
            ...
            if (ekf_count == 0) or True:
                ...
                F = np.eye(4) + dt * np.diag([1, 1], 2)
                
                l_c_2 = (ekf_COM.x[0]**2 + ekf_COM.x[1]**2)
                l_c = math.sqrt(l_c_2)
                
                """ F [2, 0] = dt * Kr * g * ekf_COM.x[1] / l_c**2
                F [2, 1] = dt * Kr * g * ekf_COM.x[0] / l_c**2
                
                if abs(ekf_COM.x[0]) >= threshold:
                    x4_x1 = ekf_COM.x[3] / ekf_COM.x[0]
                    F [2, 0] += (x4_x1)**2 * dt
                    F [2, 3] = - 2 * dt * x4_x1
                
                F [3, 0] = 2 * dt * Kr * g * ekf_COM.x[0] / l_c**2
                
                if abs (ekf_COM.x[1]) >= threshold:
                    x3_x2 = ekf_COM.x[2] / ekf_COM.x[1]
                    
                    F [3, 1] = dt * x3_x2**2
                    F [3, 2] = 2 * dt * x3_x2 """
                
                """ F [2, 0] = dt * Kr * g * (ekf_COM.x[1]**2 - ekf_COM.x[0]**2) * ekf_COM.x[1] / (l_c_2)**2
                F [2, 1] = dt * Kr * g * (ekf_COM.x[0]**2 - ekf_COM.x[1]**2) * ekf_COM.x[0] / l_c_2**2
                ...
                F [3, 0] = 2 * dt * Kr * g * ekf_COM.x[0] * ekf_COM.x[1]**2 / l_c_2**2
                F [3, 1] = - 2 * dt * Kr * g * ekf_COM.x[1] * ekf_COM.x[0]**2 / l_c_2**2
                ...
                """ F [2, 0] = - dt * Kr * g * ekf_COM.x[1]**3 / l_c**3
                F [2, 1] = - dt * Kr * g * ekf_COM.x[0]**3 / l_c**3
                
                
                F [3, 0] = dt * Kr * g * (ekf_COM.x[0]**3 + 2 * ekf_COM.x[0] * ekf_COM.x[1]**2) / l_c**3
                F [3, 1] = - dt * Kr * g * ekf_COM.x[1] * ekf_COM.x[0]**2 / l_c**3
                
                if abs(ekf_COM.x[0]) >= threshold:
                    x4_x1 = ekf_COM.x[3] / ekf_COM.x[0]
                    x3_x1 = ekf_COM.x[2] / ekf_COM.x[0]
                    
                    F [3, 0] -= dt * x4_x1 * x3_x1
                    F [3, 2] += 2 * dt * x4_x1
                    F [3, 3] += dt * x3_x1
                
                if abs (ekf_COM.x[1]) >= threshold:
                    x3_x2 = ekf_COM.x[2] / ekf_COM.x[1]
                    x4_x2 = ekf_COM.x[3] / ekf_COM.x[1]
                    
                    F [2, 1] += dt * x3_x2 * x4_x2
                    F [2, 2] -= dt * x4_x2
                    F [2, 3] -= dt * x3_x2 """
                ...
                F [2, 0] = - dt * (x[0]**2 - x[1]**2)**2 * (g * x[1] - x[2]**2 - x[3]**2) / l_c_2**2
                
                F [2, 1] = - dt * x[0] * ((x[0]**2 - x[1]**2) * g - 2 * x[1] * (x[2]**2 + x[3]**2)) / l_c_2**2
                
                F [2, 2] -= 2 * dt * x[0] * x[2] / l_c_2
                
                F [2, 3] -= 2 * dt * x[0] * x[3] / l_c_2
                
                
                F [3, 0] = - 2 * dt * x[0] * x[1] * (x[1] * g - x[2]**2 - x[3]**2) / l_c_2**2
                
                F [3, 1] = - dt * ((x[0]**2 - x[1]**2) * (x[2]**2 + x[3]**2) - 2 * x[0]**2 * x[1] * g) / l_c_2**2
                
                F [3, 2] -= 2 * dt * x[1] * x[2] / l_c_2
                
                F [3, 3] -= 2 * dt * x[1] * x[3] / l_c_2
                ...
                #acc_data.shape = (3, 1)
                #print (data.sensor ("IMU_acc").data)
                #q_est = ekf_COM.x[0].copy()
                #q_est = data.qpos[0]
                q_est = math.atan(- ekf_COM.x[0] / ekf_COM.x[1])
                ...
                #print (z)
                #print ('pre-update:', ekf_COM.x)
                #print ('measures: ', z)
                
                #print ('diff: ', z - h_x(ekf_COM.x.copy()))
                #meas_diff.append(z - h_x(ekf_COM.x.copy()))
                meas_val.append(z.copy())
                
                est_meas.append(h_x(ekf_COM.x))
                
                
                
                ekf_COM.predict_update(z, HJacobian = H_jac, Hx = h_x)
                #print ('post update: ', ekf_COM.x)
                #ekf_COM.predict_update(z_sim, HJacobian = H_jac, Hx = h_x)
                #ekf_COM.predict_update(data.sensor("IMU_gyro").data[1], HJacobian = H_jac, Hx = h_x)
                #ekf_COM.predict_update(data.qvel[0], HJacobian = H_jac_ang, Hx = h_x_ang)
            ...
            c_est = ekf_COM.x[:2].copy()
            
            """ ang_err.append (data.qpos[0].copy() - q_est)
            ang_vels.append (data.qvel[0].copy() - ekf_COM.x[-1].copy())
            
            
            ang_est.append (ekf_COM.x[0])
