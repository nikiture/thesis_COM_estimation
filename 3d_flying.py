...
while viewer.is_running() and not exit:
    ...
    if not pause:
        ...
        for i, T_i in enumerate(T_i_i):
            ...
            mujoco.mju_mulMatVec(C_0_i [i], T_0_i, C_i_i [i])
            

            
            #print (data.cinert [i + 1, 9])
            #C_0_t += data.cinert [i + 1, 9] * C_0_i [0:3]
            C_0_t += data.body(main_body_names[i]).cinert[9] * C_0_i [i, 0:3]
        ...
        C_0_t += data.body(main_body_names[0]).cinert[9] * C_i_i [0, 0:3]
        C_0_t /= model.body_subtreemass[0]
        ...
        for i in range (0, len(l)):
            mujoco.mjv_initGeom(viewer.user_scn.geoms[i + 1],\
                type = mujoco.mjtGeom.mjGEOM_LINEBOX, size = .04 * np.ones(3),\
                pos = C_0_i [i, 0:3], mat = np.eye(3).flatten(), rgba = blue_color)
