...
model = mujoco.MjModel.from_xml_path("models/3D_2R_pend.xml")
data = mujoco.MjData(model)

#joint_names = ["ground_joint", "middle_joint", "prop_base_joint"]
joint_names = ["prop_base_joint_1", "prop_base_joint_2", "knee"]
...
data.qpos[0] = start_angle_2
data.qpos[1] = start_angle_3
data.qpos[2] = start_angle_4
...
l = np.append(l, mujoco.mju_dist3(data.joint(joint_names[0]).xanchor, data.joint(joint_names[1]).xanchor))
...
T_0_1 [:3, 3] = data.body(main_body_names[0]).xpos.copy()
...
with mujoco.viewer.launch_passive(model, data, key_callback = kb_callback) as viewer:
    ...
    while viewer.is_running() and not exit:
        ...
        if not pause:
            ...
            T_1_i = np.eye(4)
            C_1_t = np.zeros(4)
            R_i_i = np.zeros((model.njnt + 1, 3, 3))
            
            for i in range (0, len(R_i_i)):
                R_i_i[i] = np.eye(3)
            ...
            for j, T_i in enumerate(T_i_i[1:]):
                ...
                #print (i)
                mujoco.mju_mulMatMat(R_curr, T_i [0:3, 0:3], R_i_i [j])
            ...
            T_0_1_curr [:3, 3] = data.body(main_body_names[0]).xpos.copy()
